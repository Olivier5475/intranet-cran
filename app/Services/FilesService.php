...
readonly class FilesService implements Interfaces\FilesServiceInterface {
    ...
    public function read($id) : ?string {
        ...
        if(empty($path)) {
            throw new Filesystem\FileNotFoundException("chemin d'accès vide");
        }
        ...
        try {
            ...
            Log::warning("File not found");
            throw new Filesystem\FileNotFoundException("Fichier introuvable");
        }
    }

    public function update(int $id, array $data): bool|FileDTO {
        if(empty($id) || empty($data["name"] || empty($data["type"] || empty($data["departement"])))) {
    ...
    private function makeFileDTO(File $file): FileDTO {
        $departements = [];
//        foreach ($file->departements() as $departement) {
