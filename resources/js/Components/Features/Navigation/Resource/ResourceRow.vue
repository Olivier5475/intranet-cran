...
<template>
    ...
    >
        <component v-if="!activeRename" :is="child.type !== 'file' ? Link : 'a'" :href="links.href" :target="child.type == 'file' ? '_blank' : ''"
            ...
            <p class="text-sm font-medium text-gray-700 dark:text-zinc-200 break-words">{{ decodeEntities(child.name) }} <span v-if="child.deadline">(Fin : {{ formatForDisplay(child.deadline) }})</span></p>
        </component>
