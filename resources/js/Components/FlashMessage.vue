...
import { CheckCircleIcon, XCircleIcon, ExclamationTriangleIcon, XMarkIcon } from '@heroicons/vue/20/solid';
...
const config = {
    success: { color: 'text-emerald-500', bg: 'bg-emerald-50/90 dark:bg-emerald-900/20', border: 'border-emerald-200 dark:border-emerald-800', icon: CheckCircleIcon },
    warn: { color: 'text-amber-500', bg: 'bg-amber-50/90 dark:bg-amber-900/20', border: 'border-amber-200 dark:border-amber-800', icon: ExclamationTriangleIcon },
...
watch(() => [page.props.flash.success, page.props.flash.error, page.props.flash.warn], ([success, error, warn]) => {
    ...
    if (msg) {
        ...
        type.value = success ? 'success' : warn ? 'warn' : 'error';
        show.value = true;
        setTimeout(() => show.value = false, 4000); // Un peu plus long pour laisser le temps de lire
    }
}, { immediate: true });
</script>

<template>
    <div class="fixed top-6 right-6 z-[100] w-full max-w-sm pointer-events-none">
        <Transition
        ...
        >
            <div v-if="show"
                 :class="[
                    'pointer-events-auto overflow-hidden rounded-2xl border backdrop-blur-md shadow-2xl',
            ...
            >
                ...
                <div class="h-1 w-full bg-gray-200 dark:bg-zinc-700/50">
                    <div
