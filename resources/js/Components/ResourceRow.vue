...
<template>
    <div
        class="group py-3 px-4 border-gray-100 dark:border-zinc-800 hover:bg-sky-50/50 dark:hover:bg-slate-900/10 grid grid-cols-12 items-center border-t transition-colors duration-150"
    >
        <component :is="child.type !== 'file' ? Link : 'a'" :href="links.href" class="space-x-3 col-span-5 flex items-center overflow-hidden">
        ...
        <div class="text-xs text-gray-500 dark:text-zinc-400 col-span-2 text-center">
        ...
        <div class="relative col-start-12 flex justify-end" v-if="canEdit" @mouseenter="isMenuExpend = true" @mouseleave="isMenuExpend = false">
            <button
                @click="toggleMenu = !toggleMenu"
                class="p-1 hover:bg-gray-100 dark:hover:bg-zinc-700 rounded-full transition-all group-hover:opacity-100"
            >
            ...
            >
                <Link
                    v-if="links.history"
                    :href="links.history"
                    class="px-4 py-2 text-xs hover:bg-gray-50 dark:hover:bg-sky-900/50 text-sky-500 block"
                    >Historique</Link
                >
