...
const page = usePage();
const userDepartementsIds = page.props.auth.user.departements_ids;
...
onMounted(() => {
    if (!props.folder) {
        ...
        form.departements = allAvailableDeps.filter((id) => userDepartementsIds.includes(id));
    }
});

const showExternalWarning = computed(() => {
    return form.departements.some((selectedId) => !userDepartementsIds.includes(selectedId));
});
        ...
        <form @submit.prevent="submit" class="space-y-8">
            <div class="md:flex-row gap-4 flex flex-col">
                <div class="space-y-2 flex-grow">
                    <label class="font-black text-gray-400 ml-1 text-[10px] tracking-[0.2em] uppercase">Nom du dossier</label>
                    <input
                        type="text"
                        v-model="form.name"
                ...
                <div v-if="page.props.auth.user.role == 'admin'" class="md:w-1/4 space-y-2">
                    <label class="font-black text-gray-400 ml-1 text-[10px] tracking-[0.2em] uppercase">Couleur UI</label>
